...
document.addEventListener("DOMContentLoaded", function() {
    // --- Configuração e Lógica de Cálculo (Adaptado de firebase_analise.js) ---
    const DIMENSOES_CONFIG = {
        ...
        // Comportamentos Ofensivos (73-76) e Fatores Adicionais (77-87) podem ser adicionados se necessário
    };
    ...
    // --- Lógica de Geração de Recomendações (Exemplo) ---
    // Esta função precisa ser adaptada com recomendações reais e mais detalhadas
    function gerarRecomendacoes(dimensoes) {
        ...
        dimensoes.forEach(dimensao => {
            const rec = {
                ...
                acoes: [] // Adicionar ações padrão ou baseadas em regras
            };

            // Exemplo simples de ações genéricas
    ...
    try {
        // Inicializa o Firebase (usa a configuração de firebase_config.js)
        if (typeof firebase !== 'undefined' && typeof firebaseConfig !== 'undefined') {
            firebase.initializeApp(firebaseConfig);
            const database = firebase.database();
            const respostasRef = database.ref('respostas'); // Assume que as respostas estão em '/respostas'

            respostasRef.on('value', (snapshot) => {
                const dadosFirebase = snapshot.val();
                const respostasArray = dadosFirebase ? Object.values(dadosFirebase) : [];

                // Calcula os resultados agregados
                const resultados = calcularResultadosCOPSOQ(respostasArray);

                if (resultados) {
                    // Gera as recomendações
                    const recomendacoes = gerarRecomendacoes(resultados.dimensoes);

                    // Atualiza o HTML do relatório com os dados
                    atualizarRelatorioHTML(resultados, recomendacoes);
                } else {
                    // Exibe mensagem se não houver dados
                    document.body.innerHTML = '<div class="container text-center mt-5"><p>Ainda não há dados suficientes para gerar o relatório.</p></div>';
                }
            }, (error) => {
            ...
            console.error("Firebase ou firebaseConfig não definidos. Verifique a inclusão dos scripts.");
    ...
    // --- Função para Atualizar o HTML do Relatório ---
    function atualizarRelatorioHTML(resultados, recomendacoes) {
        ...
        const categoriasData = categoriasLabels.map(label => resultados.categorias[label]);
        ...
        if (dimensoesCriticas.length > 0) {
            ...
            dimensoesCriticasContainer.innerHTML = '<p>Nenhuma dimensão crítica identificada.</p>';
            recomendacoesPrioritariasContainer.innerHTML = '<p>Nenhuma recomendação prioritária identificada.</p>';
        }
        ...
        if (dimensoesIntermediarias.length > 0) {
            ...
            dimensoesIntermediariasContainer.innerHTML = '<p>Nenhuma dimensão intermediária identificada.</p>';
            recomendacoesSecundariasContainer.innerHTML = '<p>Nenhuma recomendação secundária identificada.</p>';
        }
        ...
        if (dimensoesPositivas.length > 0) {
            ...
            dimensoesPositivasContainer.innerHTML = '<p>Nenhuma dimensão positiva identificada.</p>';
            acoesManutencaoContainer.innerHTML = '<p>Nenhuma ação de manutenção identificada.</p>';
        }
        ...
        resultados.dimensoes.forEach(dimensao => {
            ...
            // Adicionar classe de cor ao texto ou célula, se necessário
